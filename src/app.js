...
app.post('/api/insert/:tableName', async (req, res) => {
  try {
    ...
    // Verificar si faltan columnas necesarias, excluyendo las columnas incrementales
    const missingColumns = tableColumns.filter(column => !columns.includes(column) && !identityColumns.includes(column));
...
app.get('/api/select/:tableName', async (req, res) => {
  try {
    ...
    if (result.recordset.length > 0) {
      res.status(200).send({ status: 200, message: 'Right Query', data: result.recordset }); // OK with data
    } else {
      res.status(200).send({ status: 204,  message: 'Empty Query', data: [] }); // No data found
    }
  } catch (err) {
    console.error(err);
    res.status(500).send({ status: 500, message: 'Internal Server Error' }); // Connection or query error
  } finally {
    sql.close();
  }
});
